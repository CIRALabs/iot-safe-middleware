...
*
*/
int computeSignature(ROT* pROT, std::vector<uint8_t> hash_val, std::vector<uint8_t> &sign_res) {
    ...
    int result = pROT->signInit(keyId, CONTAINER_ID_LENGTH, rot_algo);
    _log( PY_LOG_LEVEL_DEBUG, "\n-->Running AppletTests - VerifySignature: SignInit\n");
    if (result == 0) {
        _log( PY_LOG_LEVEL_DEBUG, "\n-->Running AppletTests - VerifySignature: SignFinal\n");
        uint8_t * out_sign = sign_res.data();
        uint16_t out_len = sign_res.size();           
        result = pROT->signFinal((const uint8_t*)hash_val.data(), (uint16_t)hash_val.size(), out_sign, &out_len);
        if (result == 0)
        {
...
int computeSignatureC(ROT* pROT, uint8_t *hash, uint16_t hash_len, uint8_t* out_sign, uint16_t *out_len) {
    ...
    if (result == 0) {
        _log( PY_LOG_LEVEL_DEBUG, "\n-->Running AppletTests - VerifySignature: SignFinal\n");
        result = pROT->signFinal(hash, hash_len, out_sign, out_len);
...
{
    uint16_t pub_key_max_len=512; // Sufficient for P256 R1 Key
    uint8_t* pp_pub_key = (uint8_t*)malloc(pub_key_max_len);
    uint16_t pub_key_len; // Sufficient for P256 R1 Key
    ...
    result = _rot->getCertificateByContainerId(containerId, CONTAINER_ID_LENGTH, pub_key_max_len, pp_pub_key, &pub_key_len);
