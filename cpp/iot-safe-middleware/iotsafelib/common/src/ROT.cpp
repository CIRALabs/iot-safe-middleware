...
static uint32_t tlvParserLength(uint8_t*, uint32_t*);
...
int ROT::readFile(const uint8_t *path, uint16_t pathLen,
                  ...
                  uint16_t u16MaxLen, uint8_t *data, uint16_t *dataLen)
{
        ...
        getStatusWord() == SW_EXECUTION_OK)
    {
        ...
        // We allocate a temporary buffer to read one line at the
        // time. If we bust our "u16MaxLen" we fill what we can and
        // return an error
        uint8_t u8aReadBuffer[READ_LINE_LEN];
        while (offset + READ_LINE_LEN < u16MaxLen)
        {
            ...
            int rc = transmit(_channel, 0xB0, p0, p1, cmd, cmdLen, 0);
            if( !rc ) {
                // FIXME: This should be a logged call
                _log( PY_LOG_LEVEL_ERROR, "Error while transmitting command\n");
                result = ERR_INVALID_RESPONSE;
                break;
            } else if( getStatusWord() == SW_CONDITION_NOT_SATISFIED ) {
                _log( PY_LOG_LEVEL_ERROR, "%s%s%s",
                        "Error while reading the file. The conditions were not satisfied.\n",
                        "This can either be because the read is not granted or because the\n",
                        "file is deactivated.\n");
                result = ERR_INVALID_RESPONSE;
                break;
            } else if( getStatusWord() != SW_EXECUTION_OK ) {
                _log( PY_LOG_LEVEL_ERROR, "There was an error of type: %x\n", getStatusWord());
                result = ERR_INVALID_RESPONSE;
                break;
            } else {
                uint16_t u16TempSize = getResponse( u8aReadBuffer );
                if( u16TempSize + offset > u16MaxLen ) {
                    _log( PY_LOG_LEVEL_ERROR, "Not enough memory to read buffer" );
                    memcpy( data + offset, u8aReadBuffer, u16MaxLen - offset - u16TempSize );
                    result = ERR_OUT_OF_MEMORY;
                    break;
                } else {
                    memcpy( data + offset, u8aReadBuffer, u16TempSize );
                    result = ERR_NOERR;
                    offset += u16TempSize;
                }
            }
...
/** Public *******************************************************************/
int ROT::getCertificateByContainerId(const uint8_t *containerId, uint16_t containerIdLen, uint16_t u16MaxLen, uint8_t *cert, uint16_t *certLen)
{
    if( containerId == NULL ) {
        return ERR_INVALID_PARAMETERS;
    } else if( cert == NULL ) {
        return ERR_INVALID_PARAMETERS;
    } else if( certLen == NULL ) {
        return ERR_INVALID_PARAMETERS;
    } else if( u16MaxLen > MAX_CERTIFICATE_LEN ) {
        _log( PY_LOG_LEVEL_ERROR, "Certificates cannot exceed a length of %d", MAX_CERTIFICATE_LEN );
        return ERR_INVALID_PARAMETERS;
    }

    // We display the container ID we want to use
    char stringBuffer[100];
    uint16_t u16Offset = snprintf( stringBuffer, sizeof(stringBuffer), "getCertificateByContainerId: ");
    for (int i = 0; i < containerIdLen; i++)
    {
        u16Offset += snprintf(&stringBuffer[u16Offset], sizeof(stringBuffer) - u16Offset, "%02X", *(containerId + i));
    }
    snprintf( &stringBuffer[u16Offset], sizeof(stringBuffer) - u16Offset, "\n");
    _log( PY_LOG_LEVEL_DEBUG, "%s", stringBuffer );

    return readFile(
        const_cast<uint8_t *>(AID),
        sizeof AID,
        containerId,
        containerIdLen,
        nullptr,
...
extern "C" int ROT_get_certificate_by_container_id(
            ROT* rot,
            const uint8_t *container_id,
            uint16_t containerIdLen,
            uint16_t u16MaxLen,
            uint8_t *cert,
            uint16_t *cert_len) {

    return  rot->getCertificateByContainerId(
            container_id,
            containerIdLen,
            u16MaxLen,
            cert,
...
extern "C" int ROT_generate_random(ROT* rot, uint8_t* data, uint16_t dataLen) {
